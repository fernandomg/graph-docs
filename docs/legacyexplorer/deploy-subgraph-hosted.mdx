...
If you have not checked out already, check out how to write the files that make up a [subgraph manifest](https://thegraph.com/docs/developer/create-subgraph-hosted#the-subgraph-manifest) and how to install the [Graph CLI](https://github.com/graphprotocol/graph-cli) to generate code for youe subgraph. Now, it's time to deploy your subgraph to the Legacy Explorer, also known as the Hosted Service. 

## Create a Legacy Explorer account

Before using the Legacy Explorer, create an account in our Legacy Explorer. You will need a [Github](https://github.com/)
account for that; if you don't have one, you need to create that first. Then, navigate to the [Legacy Explorer](https://thegraph.com/legacy-explorer/), click on the _'Sign up with Github'_ button and complete Github's authorization flow.

## Store the Access Token

After creating an account, navigate to your [dashboard](https://thegraph.com/legacy-explorer/dashboard). Copy the access token
...
Before deploying the subgraph, you need to create it in The Graph Explorer. Go to the [dashboard](https://thegraph.com/legacy-explorer/dashboard)
...
Graph Node exposes a graphql endpoint which you can query to check the status of your subgraph. On the Legacy Explorer, it is available at `https://api.thegraph.com/index-node/graphql`. On a local node it is available on port `8030/graphql` by default. The full schema for this endpoint can be found [here](https://github.com/graphprotocol/graph-node/blob/master/server/index-node/src/schema.graphql). Here is an example query that checks the status of the current version of a subgraph:
