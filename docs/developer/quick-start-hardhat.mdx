...
```
in a terminal. It will create 10 test accounts with some Ether on all of them. Note that `--hostname 0.0.0.0` is necessary for Hardhat network to be accessible from within Docker and from other machines.
...
After running `graph init` in step 3, you can now deploy the example smart contract that the subgraph indexes to Hardhat network.
...
in the root directory of the subgraph from step 3. This will compile the `GravatarRegistry` smart contract and deploy it to Hardhat network. It will also execute a couple of transactions against the contract so that there is data to index.
