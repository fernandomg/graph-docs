---
id: create-subgraph-hosted
title: Create a Subgraph
---

Before being able to use the Graph CLI, you need to create your subgraph in [Subgraph Studio](https://thegraph.com/studio) or in the [Legacy Explorer](https://thegraph.com/legacy-explorer). You will then be able to setup your subgraph project and deploy it to the platform of your choice.

The `graph init` command can be used to set up a new subgraph project, either from an existing contract on any of the public Ethereum networks, or from an example subgraph. This command can be used to create a subgraph for the legacy explorer or for our new Subgraph Studio by passing two different options `graph init --product hosted-service` for the legacy explorer and `graph init --product subgraph-studio` for Subgraph Studio.
...
Additional networks are supported on the Hosted Service:
...
```
graph init --from-example --product subgraph-studio <SUBGRAPH_SLUG> [<DIRECTORY>]
```
...
| `BitInt`     | Large integers. Used for Ethereum's `uint32`, `int64`, `uint64`, ..., `uint256` types. Note: Everything below `uint32`, such as `int32`, `uint24` or `int8` is represented as `i32`. |
...
In the next section, we will describe how to deploy your subgraph using the
Graph Explorer.
...
> **Note:** Call handlers are not supported on Rinkeby and Ganache. Call
> handlers depend on the Parity tracing API and neither Rinkeby nor Ganache
> support this (Rinkeby is Geth-only).
...
service](https://thegraph.com/legacy-explorer), this is
